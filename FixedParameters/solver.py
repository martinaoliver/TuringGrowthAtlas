...
class Solver:  # Defines iterative solver methods
    ...
    def hill_equations_neg(interaction, rate, n):
        # Returns hill equation as a lambda function for a specified interaction
        ...
        if interaction == 1:
            ...
            def func(conc):
                if conc < 0:
                    return 1/(1-((rate/conc)*(-1))**n)
                else:
                    return 1/(1 + (rate/conc)**n)
    ...
    def fourier_classify(U,L,dx, threshold = 2, plot = False):
